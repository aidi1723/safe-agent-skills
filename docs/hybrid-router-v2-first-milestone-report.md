...
| `bash scripts/verify.sh` | 0 | 318 tests passed; schemas, maintenance, v1 eval, v2 eval, contract gate, invariant stage/edge and overlap-policy acceptance, and smoke routes passed |
...
selection.
Overlap group IDs are also required to be nonempty strings before duplicate or
reference checks, preventing non-hashable IDs from escaping the controlled v2
error boundary.
