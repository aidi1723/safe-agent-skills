#!/usr/bin/env python3
"""
Phase 4 evaluation script for Router v3 Bundle v2 acceptance testing.

Compares Phase 4 (Bundle v2 with conditional logic) vs Oracle.
"""
...
def run_v3_router_phase4(task_description: str, registry: Path, bundles: Path) -> Dict:
    """
    Run Router v3 Phase 4 with Bundle v2 on a task.
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return {
                ...
                "route_status": output.get("routing_status", "unknown"),
                "raw_output": output
            }
        else:
            return {
...
def get_oracle_selection(task: Dict) -> Dict:
    """Get oracle selection from task definition."""
    return {
...
def evaluate_single_task(
    ...
    """Evaluate a single task with Phase 4 vs Oracle."""
    task_id = task["id"]
    ...
    result = {
        ...
        "phase4": run_v3_router_phase4(description, registry, bundles),
        "oracle": get_oracle_selection(task)
    }

    # Calculate metrics
    result["metrics"] = calculate_task_metrics(result)

    return result


def calculate_task_metrics(result: Dict) -> Dict:
    ...
    # Scenario match
    phase4_scenario = phase4.get("selected_scenario")
    expected_scenario = oracle.get("selected_scenario")
    metrics["scenario_match"] = (phase4_scenario == expected_scenario)

    # Skill precision/recall/F1
    phase4_skills = set(phase4.get("selected_skills", []))
    oracle_skills = set(oracle.get("selected_skills", []))

    if len(phase4_skills) > 0:
        true_positives = len(phase4_skills & oracle_skills)
        precision = true_positives / len(phase4_skills)
    ...
    if len(oracle_skills) > 0:
        ...
        metrics["skill_recall"] = 1.0 if len(phase4_skills) == 0 else 0.0
    ...
    # Route quality
    route_status = phase4.get("route_status", "unknown")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Phase 4 evaluation for Router v3 Bundle v2"
    )
    parser.add_argument(
        "--tasks",
        type=Path,
    ...
    parser.add_argument(
        "--bundles",
        type=Path,
        default=Path("bundles/index-v2-tier1.json"),
        help="Path to Bundle v2 index (default: bundles/index-v2-tier1.json)"
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Output file for results (default: evals/phase4-results/eval-<timestamp>.json)"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    print(f"Evaluating {len(tasks)} tasks with Phase 4 Bundle v2")
    print(f"Bundle file: {args.bundles}")
    print()
    ...
    for i, task in enumerate(tasks, 1):
        print(f"[{i}/{len(tasks)}]")
        result = evaluate_single_task(task, args.registry, args.bundles)
    ...
    # Prepare output
    output_data = {
        "evaluation_name": "phase4_bundle_v2_acceptance",
        "evaluated_at": datetime.now().isoformat(),
        "task_file": str(args.tasks),
        "bundles_file": str(args.bundles),
    ...
    if args.output:
        ...
        output_file = Path(f"evals/phase4-results/eval-{timestamp}.json")
