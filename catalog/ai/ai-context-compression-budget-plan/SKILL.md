...
Use this skill when long documents, chat history, code context, retrieval
results, or memory notes must fit inside a constrained model context.

## Safe Workflow

1. Inventory source materials, task-critical facts, required citations,
   freshness constraints, and sections that must not be summarized away.
2. Separate facts, instructions, decisions, open questions, code references,
   and low-value repetition before compression.
3. Preserve identifiers, file paths, source dates, numeric values, constraints,
   and user decisions exactly when they drive the task.
4. Define compression budget, excluded material, retained evidence, and when to
   fetch the original source again.
5. Verify the compressed context against representative source samples before
   downstream use.
