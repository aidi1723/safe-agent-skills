...
description: Use when reviewing loop workflows, metadata-only skill candidates, upstream reference clusters, or local adoption drafts before catalog inclusion.
---
...
Use this draft when reviewing the `loop` metadata-only
...
- upstream candidate: `loop`
- source domain: `engineering`
- source path: `engineering/autoresearch-agent/skills/loop`
