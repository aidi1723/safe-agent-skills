...
description: Use when reviewing status workflows, metadata-only skill candidates, upstream reference clusters, or local adoption drafts before catalog inclusion.
---
...
Use this draft when reviewing the `status` metadata-only
...
- source path: `engineering/autoresearch-agent/skills/status`
