...
description: Use when reviewing brief workflows, metadata-only skill candidates, upstream reference clusters, or local adoption drafts before catalog inclusion.
---
...
Use this draft when reviewing the `brief` metadata-only
...
- upstream candidate: `brief`
- source domain: `c-level-advisor`
- source path: `c-level-advisor/c-level-agents/skills/brief`
